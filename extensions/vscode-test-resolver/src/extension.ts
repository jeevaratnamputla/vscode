	...
	function doResolve(authority: string, progress: vscode.Progress<{ message?: string; increment?: number }>): Promise<vscode.ResolverResult> {
		...
		const serverPromise = new Promise<vscode.ResolvedAuthority>(async (res, rej) => {
			...
			if (!commit) { // dev mode
				...
				extHostProcess = cp.spawn(serverCommandPath, commandArgs, { env, cwd: vscodePath, shell: false });
			} else {
				...
				extHostProcess = cp.spawn(path.join(serverLocation, 'bin', serverCommand), commandArgs, { env, cwd: serverLocation, shell: false });
			}
